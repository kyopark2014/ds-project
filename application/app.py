...
with st.sidebar:
    ...
    # radio selection
    mode = st.radio(
        label="원하는 대화 형태를 선택하세요. ",options=["일상적인 대화", "RAG", "Agent", "Agent (Chat)", "Report Agent", "이미지 분석"], index=3
    )   
    ...
    with st.chat_message("assistant"):
        if mode == '일상적인 대화':
            ...
            with st.status("thinking...", expanded=True, state="running") as status:
                ...
                response = asyncio.run(report_agent.run_report_agent(prompt, containers))
                logger.info(f"response: {response}")
                st.session_state.messages.append({"role": "assistant", "content": response})
